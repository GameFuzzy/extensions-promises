...
const MANGATX_DOMAIN = 'https://www.mangatx.com'

export const ComicExtraInfo: SourceInfo = {
  version: '1.0.0',
  name: 'MangaTX',
  description: 'Extension that pulls manga, manhua, and manhwa from MangaTX.com',
  author: 'GameFuzzy',
...
export class MangaTX extends Source {
  parser = new Parser()
  getMangaShareUrl(mangaId: string): string | null { return `${MANGATX_DOMAIN}/manga/${mangaId}` }

  async getMangaDetails(mangaId: string): Promise<Manga> {

    let request = createRequestObject({
      url: `${MANGATX_DOMAIN}/manga/${mangaId}`,
      method: 'GET'
    })
  ...
  async getChapters(mangaId: string): Promise<Chapter[]> {
    ...
    while(pagesLeft > 0)
    {
      let pageRequest = createRequestObject({
        url: `${MANGATX_DOMAIN}/comic/${mangaId}/${pagesLeft}`,
        method: "GET"
      })
  ...
  async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
    ...
    let request = createRequestObject({
      url: `${MANGATX_DOMAIN}/${mangaId}/${chapterId}/full`,
      method: 'GET',
    })
  ...
  async filterUpdatedManga(mangaUpdatesFoundCallback: (updates: MangaUpdates) => void, time: Date, ids: string[]): Promise<void> {
    ...
    while (loadNextPage) {
      ...
      let request = createRequestObject({
        url: `${MANGATX_DOMAIN}/comic-updates/${String(currPageNum)}`,
        method: 'GET'
      })
  ...
  async searchRequest(query: SearchRequest, metadata: any): Promise<PagedResults> {
    ...
    let request = createRequestObject({
      url: `${MANGATX_DOMAIN}/comic-search`,
      method: "GET",
  ...
  async getTags(): Promise<TagSection[] | null> {
    const request = createRequestObject({
      url: `${MANGATX_DOMAIN}/comic-genres/`,
      method: 'GET'
    })
  ...
  async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
    ...
    let request = createRequestObject({
      url: `${MANGATX_DOMAIN}/popular-comic`,
      method: 'GET'
    })
    ...
    request = createRequestObject({
      url: `${MANGATX_DOMAIN}/recent-comic`,
      method: 'GET'
    })
